...
class MMP_Geocoding{
	...
	*/
    ...
    public static function mapzen_geocode( $address_to_geocode ){
        ...
        if( is_array($response) && $response['response']['code'] == 200) {
            ...
            if(isset($response_body['features'])){
                ...
                return array(
                    'success'		=> true,
                    'lat'			=> $response_body['features'][0]['geometry']['coordinates'][0],
                    'lon'			=> $response_body['features'][0]['geometry']['coordinates'][1],
    ...
    public static function algolia_geocode( $address_to_geocode ){
        ...
        //info: If request successed the response must be an array and the respnse status 200
        if( is_array($response) && $response['response']['code'] == 200) {
            $response_body = json_decode($response['body'], true);
    ...
    public static function mapquest_geocode( $address_to_geocode ){
        ...
        if( is_array($response) && $response_body['info']['statuscode'] === 0) {
            ...
            if(is_wp_error($response)){
                ...
                return array(
                    'success'	=>	false,
                    'message'	=> (isset($response_body['info']['messages'][0]))?$response_body['info']['messages'][0]:'',
                );
            }
        }
    }
