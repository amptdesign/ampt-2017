...
define( 'UT_PORTFOLIO_VERSION' , '3.8.2');
...
if( !function_exists('ut_portfolio_enqueuestyles') ) :
	...
	function ut_portfolio_enqueuestyles() {
		...
		$ut_portfolio = file_exists( get_stylesheet_directory() . '/css/ut.portfolio.style' . $min . '.css' ) ? get_stylesheet_directory_uri() . '/css/ut.portfolio.style' . $min . '.css' : UT_PORTFOLIO_ASSETS_URL . '/css/ut.portfolio.style' . $min . '.css';
		...
		if( !is_admin() ) {
            ...
            if( get_option('portfolio_lightbox_script') == 'lightgallery' ) {
                ...
                wp_enqueue_style(
                    'ut-prettyphoto',
                    UT_PORTFOLIO_ASSETS_URL . 'css/plugins/prettyphoto/prettyPhoto' . $min . '.css'
                );                
	...
	function ut_portfolio_enqueuescripts() {
		...
		if( !is_admin() ) {
            ...
            if( get_option('portfolio_lightbox_script') == 'lightgallery' ) {
                ...
                wp_enqueue_script(
                    'ut-prettyphoto',
                    UT_PORTFOLIO_ASSETS_URL . 'js/plugins/prettyphoto/jquery.prettyPhoto' . $min . '.js',
                    array('jquery'), 
                    '3.1.6',
                    true
                );
            ...
            wp_enqueue_script(
                'ut-lazyload-js',
                UT_PORTFOLIO_ASSETS_URL . 'js/jquery.lazyload' . $min. '.js',
                array('jquery'),
                '1.9.1',
                false
            );
