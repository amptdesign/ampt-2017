...
(function($){
    ...
    $(document).ready(function(){
		...
		/* Lightbox Prettyphoto Effect
		================================================== */		
		if ( $().prettyPhoto ) {
            
            var pp_default_width  = 1024,
                pp_default_height = 768,
                pp_allow_resize   = true;
                
            if( $(window).width() < 1024 ) {
                
                pp_default_width  = $(window).width();          
                pp_default_height = $(window).height();
                pp_allow_resize   = true;
                
            }
            
            $('a[data-rel^="ut-prettyphoto"]').prettyPhoto({
                social_tools : false,
                deeplinking: false,
                default_width: pp_default_width,
                default_height: pp_default_height,
                allow_resize: pp_allow_resize,
        ...
        if ( $().lightGallery ) {
            ...
            $('a[data-rel^="ut-lightgallery"]').lightGallery({
                selector: 'this',
                hash: false
            });
		...
		$(document).on("click", ".ut-portfolio-link", function(event) { 
			...
			$portfolio_loader.stop(true).fadeIn( 400 , function() {				
				...
				$.scrollTo( $portfolio_wrap , 650 , {  easing: 'easeInOutExpo' , offset: -scroll_offset-40 , 'axis':'y' , onAfter : function(){ 
		...
		function utResetContent( portfolio_single_id ) {
            ...
            $("#ut-portfolio-detail-"+portfolio_single_id).find(".entry-content").fadeOut( 400 , function() {                
