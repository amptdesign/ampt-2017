...
class UT_Portfolio_Settings {
	...
	public function register_settings() {
        ...
        add_settings_field( 'portfolio_social_border' , __( 'Activate Border for Social Icons:' , 'ut_portfolio_lang' ) , array( &$this , 'portfolio_social_border' )  , 'portfolio_settings' , 'main_settings' );
        add_settings_field( 'portfolio_lightbox_script' , __( 'Lightbox Script:' , 'ut_portfolio_lang' ) , array( &$this , 'portfolio_lightbox_script' )  , 'portfolio_settings' , 'main_settings' );
        ...
        register_setting( 'portfolio_settings' , 'portfolio_social_border' , array( &$this , 'validate_field' ) );
        register_setting( 'portfolio_settings' , 'portfolio_lightbox_script' , array( &$this , 'validate_field' ) );
	...
	public function portfolio_cache_mode() {
		...
		echo '<select class="postform" name="portfolio_cache_mode">';
			echo '<option value="off" ' . selected( 'off' , $data , false ) . '>' . esc_html__('off' , 'ut_portfolio_lang') . '</option>';
			echo '<option value="on" ' . selected( 'on' , $data , false ) . '>' . esc_html__('on' , 'ut_portfolio_lang') . '</option>';			
		echo '</select>';
    ...
    public function portfolio_social_border() {
        
        $option = get_option('portfolio_social_border');
        ...
        echo '<select class="postform" name="portfolio_social_border">';
			echo '<option value="off" ' . selected( 'off' , $data , false ) . '>' . esc_html__('off' , 'ut_portfolio_lang') . '</option>';
			echo '<option value="on" ' . selected( 'on' , $data , false ) . '>' . esc_html__('on' , 'ut_portfolio_lang') . '</option>';			
		echo '</select>';
		
		echo '<p>' . __( 'Activate border for social share buttons on all your portfolio single pages.' , 'ut_portfolio_lang' );
        
    }
    
    public function portfolio_lightbox_script() {
        
        $option = get_option('portfolio_lightbox_script');
        ...
        echo '<select class="postform" name="portfolio_lightbox_script">';
			echo '<option value="prettyphoto" ' . selected( 'prettyphoto' , $data , false ) . '>' . esc_html__('PrettyPhoto' , 'ut_portfolio_lang') . '</option>';
			echo '<option value="lightgallery" ' . selected( 'lightgallery' , $data , false ) . '>' . esc_html__('LightGallery' , 'ut_portfolio_lang') . '</option>';			
		echo '</select>';
		
		echo '<p>' . __( 'Choose your desired Lightbox Popup Script.' , 'ut_portfolio_lang' );
